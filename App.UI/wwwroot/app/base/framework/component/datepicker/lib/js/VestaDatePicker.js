...
(function () {
    ...
    vestaDatePicker = function (container, element, options) {
        ...
        self.getDirection = function (strDate) {
            return getDirection();
        };
        ...
        this.setDate = function (date, cultured) {
            ...
            dateChanged(element, dateStr, calendar);
        ...
        function renderDayView(opts) {
            ...
            $(container).append(renderHeader(calendar.getMonthList()[calendar.month - 1] + " " + getNumber(calendar.year, opts.localNumbers), 'view:month', opts));
            var calTable = $("<div></div>")
                .addClass("ui-vestadp-calendar")
                .css("direction", getDirection())
                .hide();
            ...
            for (i = 0; i < 6; i++) {
                var wrow = $("<div></div>");
                for (var j = 0; j < 7; j++) {
                    var wday = $("<button type='button' class='btn btn-default btn-sm' data-event='click' data-handler='date' data-args='day:" + calendar.day + ",month:" + calendar.month + "'></button>")
                        .addClass("ui-vestadp-day")
                        .text(getNumber(calendar.day, opts.localNumbers));
                ...
                switch (handler) {
                    ...
                    case "date":
                        ...
                        if (typeof (element) !== "undefined" && !opts.showInline) {
                            ...
                            $(".ui-vestadp-selected", calTable)
                                .removeClass("ui-vestadp-selected");
        ...
        function renderHeader(title, args, opts) {
            var header = $('<div></div>')
                .addClass('ui-vestadp-header')
                .css("direction", getDirection());
            header
                .append($("<button type='button' class='btn btn-default btn-sm' data-event='click' data-handler='prev'></button>")
                .addClass("ui-vestadp-prev")
                .attr("title", opts.regional[opts.language].previous)
                .text("«"));
            header
                .append($("<button type='button' class='btn btn-default btn-sm' data-event='click' data-handler='view' data-args='" + args + "'></button>")
                .addClass("ui-vestadp-title")
                .text(title));
            header
                .append($("<button type='button' class='btn btn-default btn-sm' data-event='click' data-handler='next'></button>")
                .addClass("ui-vestadp-next")
                .attr("title", opts.regional[opts.language].next)
                .text("»"));
            return header;
        }
        ...
        function renderFooter(elm, opts) {
            var footer = $('<div></div>')
                .addClass('ui-vestadp-footer')
                .css("direction", getDirection());
            footer.append($("<button type='button' class='btn btn-success btn-sm'></button>").addClass("ui-vestadp-today").text(opts.regional[opts.language].today).click(function () {
                var today = new Date();
                var todayJd = gregorianToJd(today.getFullYear(), today.getMonth() + 1, today.getDate());
                calendar.setJulianDay(todayJd);
                var dateStr = calendar.toString(dateFormat)
            ...
            footer.append($("<button type='button' class='btn btn-danger btn-sm'></button>").addClass("ui-vestadp-clear").text(opts.regional[opts.language].clear).click(function () {
                elm.val("");
                container.slideUp("fast");
                dateChanged(element, "", calendar);
            }));
            return footer;
        }
        ...
        function renderMonth(opts) {
            currentView = 1;
            $(container).empty().addClass("ui-vestadp-container");
            $(container).append(renderHeader(getNumber(calendar.year, opts.localNumbers), 'view:year', opts));
            var calTable = $("<table cellspacing='1'></table>").addClass("ui-vestadp-calendar").css("direction", getDirection()).hide();
            var mIndex = 0;
        ...
        function renderYear(opts, year) {
            ...
            endYear = year + 7;
            year = startYear;
            $(container).append(renderHeader(getNumber(startYear, opts.localNumbers) + " - " + getNumber(endYear, opts.localNumbers), '', opts));

            for (var i = 0; i < 3; i++) {
                var yrow = $("<tr></tr>").addClass("ui-vestadp-yearlist");
                for (var j = 0; j < 4; j++) {
                    var ycell = $("<td data-event='click' data-handler='view' data-args='view:month,year:" + year + "'></td>").text(getNumber(year, opts.localNumbers));
        ...
        function getNumber(num, unicode) {
            if (!unicode || parseInt(num) < 0 || typeof (num) === "undefined")
                return num;
            return calendar.getNumber(num);
        }
        function getDirection() {
            if (self.settings.direction)
                return opts.direction;
            else
                return calendar.getDirection();
        }
        ...
        function dateChanged(element, dateStr, calendar) {

            $(element).trigger("dateChanged", [element, dateStr, calendar]);
        }
    ...
    $.fn.vestadp = function (method) {
        var methods = {
            ...
            */
            ...
            showPicker: function () {
                var vdp = methods._checkThrow(this);
                var x = $(this).data("vestadp");
                vdp.display($(this).val());
                var wrapper = $(this).closest(".datepicker-wrapper");
                var divContainer = wrapper.find("div[data-rel='vestadatepicker']");
                ...
                if (dir == "rtl")
                    ...
                    left = offset.left + "px";
                var slideDown = divContainer.is(':visible');
                if (slideDown)
                    return;
                divContainer.slideDown("fast");
                //var isStatic = ($(this).offset().top + divContainer.outerWidth()) > $(document.body).height();
                var isStatic = false;
                //divContainer.css({
