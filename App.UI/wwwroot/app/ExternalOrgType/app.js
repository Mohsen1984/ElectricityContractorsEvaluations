var basePath = '/app/ExternalOrgType';
...
app.controller("RootController",
    ["$scope", "$state", "esDatasource", "$http", "$sce", "$uibModal",
        function ($scope, $state, esDatasource, $http, $sce, $uibModal) {
            var vm = this;
            vm.success = function () {
                $state.go("ExternalOrgType.List");
            };
            vm.error = function () {
            };

            vm.ds = new esDatasource({
                url: '/ExternalOrgType/GetAllPaged',
                method:'GET',
                params: {
                    pageIndex: 0,
                    pageSize: 5
            ...
            vm.doAdvancedSearch = function () {
                vm.searchMode = 'advanced';
                vm.ds.setUrl('/ExternalOrgType/GetAllPaged');
                vm.ds.refresh();
            };
...
app.controller("DetailsController",
    ["$stateParams", "esDatasource",
        function ($stateParams, esDatasource) {
            var vm = this;
            vm.ds = new esDatasource({
                url: '/ExternalOrgType/GetById/' + $stateParams.id,
                method:'GET'
            });
            vm.ds.refresh();
        }]);
